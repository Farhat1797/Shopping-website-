...
class Web_Model extends CI_Model
{
    ...
    public function get_all_featured_product()
    {
        ...
        $info = $this->db->get();
        //index page 
        //SELECT *, `tbl_product`.`publication_status` as `pstatus` FROM `tbl_product` JOIN `tbl_category` ON `tbl_category`.`id`=`tbl_product`.`product_category` JOIN `tbl_brand` ON `tbl_brand`.`brand_id`=`tbl_product`.`product_brand` WHERE `tbl_product`.`publication_status` = 1 AND `product_feature` = 1 ORDER BY `tbl_product`.`product_id` DESC LIMIT 4
    ...
    public function get_all_new_product()
    {
        ...
        $info = $this->db->get();
        //index page 
        //SELECT *, `tbl_product`.`publication_status` as `pstatus` FROM `tbl_product` JOIN `tbl_category` ON `tbl_category`.`id`=`tbl_product`.`product_category` JOIN `tbl_brand` ON `tbl_brand`.`brand_id`=`tbl_product`.`product_brand` WHERE `tbl_product`.`publication_status` = 1 ORDER BY `tbl_product`.`product_id` DESC LIMIT 6
    ...
    public function get_all_product_pagi($limit,$offset)
    {
        ...
        $info = $this->db->get();
        //SELECT * FROM `tbl_product` JOIN `tbl_category` ON `tbl_category`.`id`=`tbl_product`.`product_category` JOIN `tbl_brand` ON `tbl_brand`.`brand_id`=`tbl_product`.`product_brand` WHERE `tbl_product`.`publication_status` = 1 ORDER BY `tbl_product`.`product_id` DESC LIMIT 8
        //echo $this->db->last_query(); exit; 
        return $info->result();
    }
